...
let citations = [
  ...
  let newCitation = () => {
    do{
      randomNumber = generateNumber(citations.length);
    } while(randomNumber == dernier) {
      citation.textContent = citations[randomNumber][0];
      auteur.textContent = citations[randomNumber][1];
      dernier = randomNumber;
    }
    
  };
  
  nouveau.addEventListener('click', newCitation);